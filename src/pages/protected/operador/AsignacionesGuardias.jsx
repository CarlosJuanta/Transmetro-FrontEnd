...
const AsignacionesGuardias = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setIsLoading(true);
      try {
        const [asigData, personalData, estacionesData] = await Promise.all([
        ...
        setEstacionesConAccesos(estacionesData);
      ...
      <div className="row">
        <div className="col-lg-8">
          <div className="d-flex justify-content-between align-items-center mb-3">
            ...
            <div className="w-50">
                <select className="form-select" value={filtroEstacion} onChange={e => setFiltroEstacion(e.target.value)}>
                    <option value="">Filtrar por Estación (Mostrar Todas)</option>
                    {estacionesConAccesos.map(est => (<option key={est.id_estacion} value={est.id_estacion}>{est.nombre}</option>))}
                </select>
            </div>
          </div>
